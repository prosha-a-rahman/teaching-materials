...
If $Y_{n} = \max\{X_{i}\}^{n}_{i = 1}$---where $X_{1},\dots,X_{n}$ is a generic sequence of independent and identically distributed random variables---then $Y_{n}$ has distribution function \begin{align}
...
Observe that, for $x < 1$, \begin{align}
...
Independence between the events $A$ and $B$ implies \begin{align}
    \mathbb{P}\{S = 2\} &= \mathbb{P} A\cap B \\
                        &= \mathbb{P}A\,\cdot\mathbb{P}B \\
                        &= 0.90\times 0.67 \\
                        &= 0.603
\end{align} Recall that independence of two events $E$ and $F$ also implies independence between their respective complements $E^{\complement}$ and $F^{\complement}$---*you should show that this is true*.
